...
import {
    ...
    DEFAULT_TTL_HOURS,
    type BrowserbaseCacheEntry,
} from "./browserbase-cache.js";

function entry(fetchedAt: string): BrowserbaseCacheEntry {
    return { fetchedAt, status: 200, contentType: "text/calendar", content: "X" };
}

describe("browserbase-cache load/save", () => {
    const dirs: string[] = [];
    afterEach(async () => {
    ...
    async function tmpFile(name = "browserbase-cache.json"): Promise<string> {
        const dir = await mkdtemp(join(tmpdir(), "bbcache-"));
        dirs.push(dir);
    ...
    it("returns an empty cache when the file is missing", async () => {
        ...
        expect(await loadBrowserbaseCache(path)).toEqual(emptyBrowserbaseCache());
    });
    ...
    it("round-trips a populated cache", async () => {
        ...
        await saveBrowserbaseCache(cache, path);
        expect(await loadBrowserbaseCache(path)).toEqual(cache);
...
describe("isFresh / getCacheTtlMs", () => {
    ...
    it("honors BROWSERBASE_CACHE_TTL_HOURS", () => {
        process.env.BROWSERBASE_CACHE_TTL_HOURS = "1";
    ...
    it("with TTL=0 every entry is stale", () => {
        process.env.BROWSERBASE_CACHE_TTL_HOURS = "0";
