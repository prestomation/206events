...
const VENUE_ADDRESS = "Spectrum Dance Theater, 800 Lake Washington Blvd E, Seattle, WA 98122";
const TICKET_URL = "https://web.ovationtix.com/trs/pe.c/36947";

/**
 * Ripper for Spectrum Dance Theater using the OvationTix (AudienceView Professional) public API.
 *
 ...
 *   - clientId: OvationTix client ID (36947 for Spectrum Dance Theater)
 *   - clientOrigin: Origin header value (https://spectrumdance.org)
 *   - lookaheadMonths: Number of months ahead to fetch (default: 6)
 */
export default class SpectrumDanceRipper implements IRipper {
    public async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        for (const cal of ripper.config.calendars) {
            const clientId = (cal.config?.clientId as number | undefined) ?? 36947;
            const clientOrigin = (cal.config?.clientOrigin as string | undefined) ?? "https://spectrumdance.org";
            ...
            try {
                ...
                calendars[cal.name].events = [{
                    ...
                    reason: `Failed to fetch Spectrum Dance Theater events: ${error}`,
                    context: undefined
                }];
            }
        }
        ...
        for (const perf of performances) {
            try {
                ...
                const event: RipperCalendarEvent = {
                    id: `ovationtix-spectrum-${perf.performanceId}`,
                    ripped: new Date(),
                    date: zonedDate,
