...
const BASE_TICKET_URL = "https://ci.ovationtix.com/35379/production";

export default class TheFeastRipper implements IRipper {
    public async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        for (const cal of ripper.config.calendars) {
            const clientId = (cal.config?.clientId as number | undefined) ?? 35379;
            const clientOrigin = (cal.config?.clientOrigin as string | undefined) ?? "https://the-feast.org";
            ...
            try {
                ...
                calendars[cal.name].events = [{
                    ...
                    reason: `Failed to fetch The Feast events: ${error}`,
                    context: undefined
                }];
            }
        }
    ...
    public parseEvents(performances: any[], timezone: any): RipperEvent[] {
        ...
        for (const perf of performances) {
            try {
                ...
                const ticketUrl = perf.productionId
                    ? `${BASE_TICKET_URL}/${perf.productionId}`
                    : "https://the-feast.org/box-office/";

                const event: RipperCalendarEvent = {
                    id: `ovationtix-the-feast-${perf.performanceId}`,
                    ripped: new Date(),
                    date: zonedDate,
