...
// DanceUS.org's Seattle swing calendar page embeds a JSON-LD array (one
// object per listed event) with reliable structured data — venue name,
...
// `/event/<id>/<slug>/` href. We merge the two by url.
const SOURCE_URL = "https://www.danceus.org/events/swing/seattle-wa-swing-calendar";
...
export default class DanceUSSwingRipper implements IRipper {
    ...
    public async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        if (!res.ok) throw new Error(`DanceUS Seattle swing calendar returned HTTP ${res.status}`);
    ...
    private buildEvent(ld: LdEvent, cardMeta: Map<string, CardMeta>): RipperEvent[] {
        ...
        // Seattle-focus filter: DanceUS's "Seattle" swing calendar includes a
        // handful of Eastside listings (Kirkland, Mercer Island). Skip those
        // rather than emit an out-of-city event.
