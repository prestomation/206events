...
/**
 * Extracts cost from Ounces Taproom event body text.
 * Pricing is in the Squarespace body (not the excerpt).
 * Dollar amount is checked before \bfree\b to handle conditional-free language correctly.
 * Style/script blocks are stripped first to avoid CSS pollution.
 */
export function extractOuncesTaproomCost(rawBody: string, summary: string): EventCost | undefined {
...
export default class OuncesTaproomRipper extends SquarespaceRipper {
    ...
    public override async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        for (const cal of calendars) {
            for (const event of cal.events) {
                ...
                const cost = extractOuncesTaproomCost(rawBody, event.summary);
