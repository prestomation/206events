...
/**
 * Extracts cost from Queer Bar event body text.
 * Pricing is in the Squarespace body (not the excerpt), e.g.:
 *   "Tickets on sale now!"
 *   "FREE! No cover, 21+"
 */
export function extractQueerBarCost(rawBody: string, summary: string): EventCost | undefined {
...
export default class QueerBarRipper extends SquarespaceRipper {
    ...
    public override async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        for (const cal of calendars) {
            for (const event of cal.events) {
                ...
                const cost = extractQueerBarCost(rawBody, event.summary);
