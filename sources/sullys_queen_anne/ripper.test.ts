...
import SullysQueenAnneRipper from './ripper.js';
...
const testDate = ZonedDateTime.parse('2026-09-16T00:00:00-07:00[America/Los_Angeles]');
...
const EVENT_WITH_IMAGE = {
    ...
    text: "Join us for National Drink Beer Day on Sept 28th and try one of our many craft brews!",
    event_date: "2026-09-28T00:00:00.000+00:00",
    all_day: false,
    start_time: "11:00",
    duration_minutes: 720,
    links: { images: [7812182] },
};

const IMAGE_LINKED = { id: 7812182, url: "//static.spotapps.co/spots/57/bf409787f44c1ab8619817b851312c/full" };
...
describe('Sullys Queen Anne Ripper', () => {
    test('parses a timed event with correct date, time, and duration', async () => {
        const ripper = new SullysQueenAnneRipper();
        ...
        expect(e.duration.toMinutes()).toBe(840);
        expect(e.id).toBe('sullys-queen-anne-2221910');
        expect(e.summary).toBe('National Cheeseburger Day');
        expect(e.location).toContain("Sully's Queen Anne");
        expect(e.url).toBe('https://sullysqueenanne.com/seattle-queen-anne-sully-s-events');
    ...
    test('resolves an event image through the linked.images join', async () => {
        ...
        expect(events).toHaveLength(1);
        expect(events[0].imageUrl).toBe('https://static.spotapps.co/spots/57/bf409787f44c1ab8619817b851312c/full');
    });
    ...
    test('emits a placeholder midnight event plus an UncertaintyError when start_time is missing on a non-all_day event', async () => {
        ...
        expect(uncertainties[0].source).toBe('sullys-queen-anne');
    ...
    test('does not drop other events when one event in the batch is malformed', async () => {
        ...
        expect(valid[0].id).toBe('sullys-queen-anne-2221910');
    });
