...
/**
 * Extracts cost from Seattle Film Society event body text.
 * Pricing is in the Squarespace body (not the excerpt), e.g.:
 *   "Tickets on sale"
 *   "Free discussion series"
 */
export function extractSeattleFilmSocietyCost(rawBody: string, summary: string): EventCost | undefined {
...
export default class SeattleFilmSocietyRipper extends SquarespaceRipper {
    ...
    public override async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        for (const cal of calendars) {
            for (const event of cal.events) {
                ...
                const cost = extractSeattleFilmSocietyCost(rawBody, event.summary);
