...
export function buildComment({ current, previous, baselineMain, reportUrl, meta, priorHistory }) {
  return buildTrendComment({
    spec: {
      ...
      intro: 'Playwright lab profile of the deployed preview (mobile, 4× CPU throttle, median of runs): post-splash main-thread blocking and tap responsiveness while the full events index lands. Warn-only — these numbers don\'t gate the PR. 🟢 = better, 🔴 = worse, ≈ = within noise.',
