...
- **`skills/event-report-triage/SKILL.md`** — Default handler for an **in-app "Report a problem" feedback issue** about an event 206.events already publishes (the `issues.opened` webhook with the `feedback`/`bug` label, e.g. `[Bug] <event name>`). **Assume the published event is wrong**, verify every field (time, date, location, duplicate status, showings, price, image) against the live source — falling back to `WebSearch` when the source host is blocked — and route each discrepancy to the right fix (ripper PR, `event-uncertainty-cache.json`, or `event-duplicate-cache.json`). Never close a report as not-actionable just because its description is blank
