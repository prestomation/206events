...
**Escalation is always one rung at a time, one PR at a time — with one exception.** If a source fails at rung 1 (CI 403), add `proxy: "outofband"` in a PR. If it still fails out-of-band (captured in the outofband report), escalate to `proxy: "browserbase"` in a follow-up PR. You must observe each failure before escalating.

**Exception — skip directly to `proxy: "browserbase"` when you can confirm a JS challenge is being served.** If the response body (from the ripper's fetch or a manual curl/fetch) is a JavaScript bot-challenge page — recognizable by `window.location.reload()`, `sgcaptcha`, Cloudflare challenge, or similar JS-redirect patterns — a plain residential-IP fetch (outofband rung 2) will get the same challenge and won't help. Go straight to `proxy: "browserbase"` in a single PR. Document the JS-challenge evidence in the PR body (e.g. "live curl returns 11 KB JS challenge page from openresty"). A source should never skip rung 2 for any other reason.
