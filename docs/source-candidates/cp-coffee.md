---
name: "C & P Coffee Company"
status: added
platform: Tribe Events ICS
url: https://candpcoffee.com/events/
tags: [Music, "West Seattle"]
firstSeen: 2026-06-19
lastChecked: 2026-06-19
pr: 0
---

C & P Coffee Company at 5612 California Ave SW, West Seattle (Admiral District). Independent coffeehouse with a rich community events calendar: live music (jazz, Brazilian, reggae, folk, acoustic world music), open mics, songwriter showcases, book clubs, mystery theater, civic events.

Investigated 2026-06-19:
- Platform: WordPress with The Events Calendar (Tribe Events) plugin
- ICS feed: `https://candpcoffee.com/?post_type=tribe_events&ical=1&eventDisplay=list`
- 30 upcoming events confirmed (June–August 2026); ~23 public events + ~7 "RESERVED FOR PRIVATE PARTY" placeholder entries
- Geo: lat 47.5516578, lng -122.3866639 (confirmed from ICS X-APPLE-STRUCTURED-LOCATION)
- Tags: Music, West Seattle
- sourceRole: venue

Added as `sources/external/cp-coffee.yaml`.
