---
name: "NCNW - Seattle Section"
status: candidate
platform: Eventbrite
url: https://www.eventbrite.com/o/ncnw-seattle-section-32936501843
tags: [Community]
firstSeen: 2026-07-11
lastChecked: 2026-08-24
pr:
---

National Council of Negro Women, Seattle Section — advocacy, education,
and service organization for Black women, families, and communities.

Investigated 2026-07-11:
- Confirmed Eventbrite organizer (id `32936501843`).
- Only 1 upcoming event at time of check: "NCNW Bethune Tea 2026: Carry
  the Torch".
- Below the "a few events, not a one-off" bar for now — re-check in a
  future cycle to see if event volume grows before implementing.

Re-checked 2026-07-22: still 0 upcoming events (Squarespace `?format=json` upcoming array empty, or Eventbrite organizer `upcomingEvents` empty). No change.

Re-checked 2026-08-24: still 0 upcoming events (`__NEXT_DATA__` `upcomingEvents` empty). No change.
