---
name: "Arcana Crafts Co / Erin's Empowered Tarot"
status: notviable
...
to `erinsempoweredtarot.com`.

Investigated 2026-08-23:
- Confirmed Squarespace (`?format=json` on `/events-calendar` returns a
  real `events-stacked` collection, `itemCount: 11`)
- All 11 items are in `past` with **stale 2021–2022 timestamps** — no
  `upcoming` entries, site appears dormant
