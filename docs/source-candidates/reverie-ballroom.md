---
name: "Reverie Ballroom"
status: candidate
platform: Squarespace
url: https://www.reverieballroom.com/events
tags: [Community, "Capitol Hill"]
firstSeen: 2026-06-30
lastChecked: 2026-09-11
---
**Reverie Ballroom** — `https://www.reverieballroom.com/events` — new wedding and event venue in the historic Oddfellows Building, 915 E Pine St, Capitol Hill (two restored ballrooms; opened 2026). Site mentions social dancing and performances in addition to private rentals.

Investigated 2026-06-30:
- Squarespace confirmed (`squarespace-cdn.com` image URLs)
- `/events?format=json` returns `itemCount: 0`, `typeName: "page"` — not a real Squarespace events collection, just a static landing page; no upcoming events listed
- New venue (opened 2026); programming likely not live yet

Re-checked 2026-07-05: `/events?format=json` still `itemCount: 0`, no change.

Re-checked 2026-07-18: `/events?format=json` still `itemCount: 0` (empty `page`
type, not an events collection), no change.

Re-evaluate when the venue announces public (non-rental) programming.

Re-checked 2026-07-22: still 0 upcoming events (Squarespace `?format=json` upcoming array empty, or Eventbrite organizer `upcomingEvents` empty). No change.

Re-checked 2026-08-10: `/events?format=json` still returns no `itemCount`/`typeName` (empty static page), 0 upcoming events. No change.

Re-checked 2026-09-09: still 0 upcoming events. No change.

Re-checked 2026-09-11: `/events?format=json` still returns an empty
`data-type="page"` shell (no `itemCount`/`typeName` for an events
collection). No change.
