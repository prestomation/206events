---
status: added
name: Seattle Folklore Society
url: https://seafolklore.org/calendar/
icsUrl: https://seafolklore.org/?post_type=tribe_events&ical=1&eventDisplay=list
tags: [Music, Arts]
sourceRole: venue
geo: null
addedPR: 713
lastChecked: 2026-06-24
---

Seattle's community folk music organization presenting traditional folk, Celtic, acoustic, and world music concerts at venues across Seattle including Phinney Center Concert Hall and Fremont Abbey.

Uses WordPress/Tribe Events. ICS feed confirmed working with 4 upcoming events in 2026 (July, September ×2, November). Multiple venues → geo: null.

Implemented as `sources/external/seattle-folklore-society.yaml`.
