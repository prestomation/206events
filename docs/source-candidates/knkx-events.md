...
**Re-evaluated 2026-09-23 (King County rule):** Issaquah items (e.g. Village Theatre) are now in scope, but that doesn't change the verdict. The `/calendar` Brightspot community calendar is a user-submitted regional round-up (HTML `PromoEvent` cards, no ICS/JSON). Much of it is still outside King County (Tacoma, Olympia, Port Townsend), and the in-area items republish venues we already cover. A custom scraper would add little new coverage.
